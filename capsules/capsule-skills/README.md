...
In the OS model, this capsule is the principal's user-space Skills index. It
discovers definitions from the workspace and principal-scoped AOS home so an
agent can load workflows contributed outside its host plugin. Capsule-owned
reference material belongs behind typed IPC tools rather than a kernel-facing
manifest protocol.
